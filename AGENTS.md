...
- **Post-Release Branch Sync Protocol:** After a pull request from `develop` to `main` is merged on GitHub, `develop` must be synchronized back with `main` (`git checkout main; git pull origin main; git checkout develop; git merge main; git push origin develop` or via `csync` alias) to keep `develop` ready for subsequent feature work.
